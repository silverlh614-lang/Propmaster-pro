...
### (2) chop 필터 A/B → **글로벌 기본 기각** (음성 결과)

가설: `chop_gate`(HTF EMA 플립 ≥8 이면 관망)로 박스권 휩쏘를 걸러 얇은 종목 PF 개선.
결과: **비균일 — 절반 개선·절반 악화, 그리고 코어(SOL)를 게이트 밑으로 떨어뜨림.**

| 방향 | 종목 (baseline expR → chop expR) |
|---|---|
| **개선** | DOT 0.261→0.303 · POPCAT 0.215→**0.289** · STX 0.146→0.164 · OP 0.159→0.170 · RENDER 0.211→0.220 · ALGO 0.133→0.143 · LDO 0.137→0.145 |
| **악화** | **SOL vbo 0.185→0.076 (PF 1.28→1.10 = 탈락)** · HYPE 0.416→0.314 · ARB vbo 0.325→0.230 · PNUT 0.196→0.172 · XRP 0.203→0.187 · ZEC 0.241→0.216 · S 0.192→0.175 |
| 무변 | ETH · AVAX · SUI · TAO · TRUMP · JTO · WLD |

- **판정: `chop_gate_flips=8` 글로벌 기본 채택 불가.** 균일 개선이 아니고, 기존 게이트
  통과 코어(SOL)를 탈락시킨다 — 청산 variant(ETH n=4)·mean_revert 와 동일한 실패 모드.
  종목별 chop on/off 는 cherry-pick → 불변식 #5 위반. **글로벌 off 유지.**
- 다만 구조 신호는 실재: chop 은 추세지속형(DOT·POPCAT·STX·RENDER)엔 +, 빠른 되돌림형
  (SOL·HYPE·ARB·PNUT)엔 −. "종목 추세성 지표로 필터 여부를 게이트가 판정"하는 방식은
  후속 가설로 남긴다 (지금은 채택 안 함).
