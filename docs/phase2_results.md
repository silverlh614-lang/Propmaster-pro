...
## 밈/내러티브 유니버스 × VWAP 되돌림 트랙 → 기각 (2026-08-01)
...
### 결과 (12mo, 신규 9종) — 게이트 1/9, 통계적으로는 0/9

| 종목 | 거래 | expR | PF | 95% CI (sd 1.4 가정) | 게이트 |
|---|---|---|---|---|---|
| MOODENG | 32 | +0.402 | 1.64 | [−0.08, +0.89] | **통과** |
| WIF | 12 | +0.395 | 1.65 | [−0.40, +1.19] | 거래<20 |
| FARTCOIN | 102 | +0.125 | 1.19 | [−0.15, +0.40] | PF<1.2 |
| AIXBT | 96 | +0.113 | 1.16 | [−0.17, +0.39] | PF<1.2 |
| KAITO | 88 | −0.070 | 0.89 | [−0.36, +0.22] | |
| PENGU | 81 | −0.074 | 0.88 | [−0.38, +0.23] | |
| DOGE | 66 | −0.107 | 0.83 | [−0.44, +0.23] | |
| VIRTUAL | 98 | −0.146 | 0.78 | [−0.42, +0.13] | |
| GRASS | 21 | −0.368 | 0.52 | [−0.97, +0.23] | |

**기각 근거 — 통과한 1종이 노이즈임을 세 방향에서 확인:**

1. **통합 기대값이 정확히 0.** 9종 596거래 거래가중 expR = **−0.0001R**(합계 −0.05R).
2. **노이즈 깔때기.** 거래수 vs |expR| 상관 **−0.90**. 통계적 힘이 있는 6종(60건 이상)은
   평균 −0.026R(−0.146~+0.125)로 0 주위에 수렴하고, 극단값은 표본이 작은 3종
   (WIF 12·GRASS 21·MOODENG 32)에만 나타난다. 엣지 신호가 아니라 분산의 부채꼴.
3. **최댓값이 우연 기대치와 같다.** 엣지 0 인 9종을 스캔할 때 우연히 기대되는 최고
   성적은 +0.368R(9표본 최댓값 기대). 실측 최고 MOODENG +0.402R — 사실상 동일.
   9종 **전부** 95% CI 가 0을 포함한다.

**채택해도 무의미:** MOODENG 은 12mo 32거래 = 0.088건/일. 현행 2건/일 포트폴리오에
4% 를 더할 뿐 통과율에 영향이 없다. 9종 중 1종만 취하는 것은 07-22 에 POPCAT·WLD 를
두고 내린 판단과 동일한 cherry-pick(불변식 #5).

**페이드 축 종결 — 4번째 수렴.** retest(07-22)·mean_revert(07-18)·vwap 로스터(07-22)에
이어 **유니버스를 바꿔도** 같은 결론이 나왔다. 07-22 의 가설("유니버스가 추세성으로
선별돼 페이드가 진다")은 이번 스캔으로 **부분 반증**된다 — 밈/잡음 유니버스에서도
페이드는 이기지 못했다. 남는 설명은 유니버스가 아니라 **우리 청산 FSM(2R 추세형
+ 부분익절 + ATR 트레일)이 페이드 진입과 구조적으로 불화한다**는 쪽이다.
페이드·되돌림·평균회귀 계열은 이 트랙에서 **종결**한다. 재개하려면 청산 FSM 자체를
바꾸는 별도 설계가 선행돼야 하고, 그것은 불변식 #3(전략-집행 분리) 재설계 사안이다.

코드 변경 없음(스캔 전용 후보 풀 그대로, 거래 경로 차단 유지). 다음 축은
**리스크 관문 A/B** — 백테스트 baseline 0.20R 은 `_PermissiveRisk` 로 관문을 끈
숫자이고(engine.py 도입부에 명시), 라이브 실측 0.09R 과의 격차를 설명할 수 있는
유일한 미검증 구조 차이다.

## 라이브 구간 재생 진단 (2026-08-01) — 구현누수 가설 기각, 레짐으로 확정

운영자 문제제기("승률도 수익률도 좋지 않다")에 대해 추측을 끝내려고, 라이브가 실제로
돈 기간을 **같은 엔진으로 재생**해 대조했다. `?daily=1` 로 7월 미게시 월별 아카이브를
일별로 메워 구간을 온전히 덮었다 (`scan?months=2&daily=1`).

### 라이브 실측 (trades_all_20260801.csv, 07-08~08-01, 청산 39건)

- 합계 **−0.15R** (평균 −0.0037R) · 순손익 **−$302.46** · 수수료 $135.72
- 승률 35.9% vs **손익분기 승률 36.0%** — 정확히 분기선. 평균 승 +1.800R / 패 −1.014R
- **달러가 R보다 나쁜 이유 = 사이징 궤적**: 전반 19건 평균리스크 $42.43·−0.431R→−$396,
  후반 20건 $27.02·**+0.402R**→+$94. 패배 거래 평균리스크 $37.40 > 승리 $29.39.
  예산 사이징의 볼록 설계(드로다운 시 자동 축소) 대가 — 07-28 결정대로이며 재론 않는다.
- **수수료 = 거래당 0.101R** (총 $135.72). 수수료 제외 시 평균 +0.0971R. 수수료/R 은
  스탑 거리에 반비례(상관 **−0.84**): BTC 스탑 0.62% → 편도 8.8%, JTO 4.35% → 1.3%.
  단, 백테스트도 같은 `PositionManager` 로 동일 수수료를 물므로 baseline 대비
  부진의 설명은 되지 못한다(양쪽에 이미 포함).

### 같은 구간 백테스트 (23종 × 2전략, 631거래)

| 전략 | 거래 | 거래가중 expR | 양수 종목 |
|---|---|---|---|
| prop_breakout | 403 | **−0.1218R** | 8/23 |
| vbo | 228 | **−0.0067R** | 12/23 |
| 전체 | 631 | **−0.0802R** | |

**라이브가 자기 백테스트를 이겼다** — prop_breakout 라이브 −0.008R(31건) vs 백테스트
−0.122R, vbo 라이브 +0.172R(7건) vs 백테스트 −0.007R.

### 판정

1. **레짐 확정.** 6~7월은 브레이크아웃이 원래 지는 구간이었다(631거래 −0.08R).
   라이브 −0.004R 은 전략 실패가 아니라 **나쁜 구간을 최소 손실로 통과한 결과**다.
2. **구현 누수 가설 기각.** 백테스트는 `_PermissiveRisk` 로 관문이 **꺼진** 상태인데도
   라이브보다 나빴다. 관문은 엣지를 갉아먹기는커녕 손실을 줄였다 → **리스크 관문 A/B
   제안 철회**(동시성·오픈리스크·예산 가드 포함). 라이브 표본이 작아(31·7건) "이겼다"
   자체는 노이즈일 수 있으나, **라이브가 백테스트보다 나빴다는 증거는 없다**는 결론은
   견고하다.
3. **로스터는 붕괴가 아니라 회전.** 12mo 평균 +0.200R → 6~7월 +0.107R, 22종 중 10종이
   수준 유지. 개선 ETH +0.397·ALGO +0.277·OP +0.273·TRUMP +0.211 / 악화 POPCAT −0.642·
   SUI −0.475·TAO −0.386. **ETH 드리프트 워치 항목 해소** (0.138 → +0.535, 로스터 최상위).
   종목당 9~22거래라 개별 값으로 로스터를 손대지 않는다 — 12mo 게이트가 결정한다(불변식 #5).

**조치: 없음. 현행 유지하고 표본을 쌓는다.** 40거래 구간의 95% CI 는 [−0.34, +0.52]R 로
0.20R baseline 을 포함하며, 0.09R 과 0.20R 을 구분하려면 ~1,237거래(하루 2건 기준 20개월)가
필요하다. 이 구간에서 파라미터를 만지는 것이 정확히 07-28 이 금지한 드로다운 중 손튜닝이다.
