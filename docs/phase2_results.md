...
## 낮은 타겟(rr_target 1.5) A/B 가설 등록 (2026-07-27) — 스캔 대기
...
```
**채택 기준**: 균일(또는 최소 비악화) 개선 — 로스터 코어(SUI·ZEC·RENDER·ARB(vbo)·
HYPE(vbo) 등) 탈락 시 기각. 거래수 급감(3R variant 실패 모드) 시 기각. 현재 미채택
(등록만) — 결과 JSON 붙여넣기로 판정.
