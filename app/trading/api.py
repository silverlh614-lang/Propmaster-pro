...
def signals(limit: int = 100, symbol: str | None = None):
    ...
    return {"stats": MANAGER.signals.stats(), "rows": MANAGER.signals.tail(limit, symbol=sym),
            "counterfactual": MANAGER.signals.counterfactual(symbol=sym), "excursion": MANAGER.signals.excursion(symbol=sym), "by_symbol": MANAGER.signals.breakdown("symbol"), "by_strategy": MANAGER.signals.breakdown("strategy")}
